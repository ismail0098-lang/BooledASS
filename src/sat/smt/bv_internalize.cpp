...
#define if_unary(F) if (a->get_num_args() == 1) { internalize_un(F); break; }
    ...
    */
    ...
    void solver::internalize_carry(app* n) {
        ...
        if (m_jointly_internalized.contains(n->get_id())) {
            return;
        }

        expr* l1_expr = n->get_arg(0);
        expr* l2_expr = n->get_arg(1);
        expr* l3_expr = n->get_arg(2);

        app* xor3_node = m.mk_app(bv.get_fid(), OP_XOR3, l1_expr, l2_expr, l3_expr);

        ctx.push(restore_vector(m_jointly_internalized));
        m_jointly_internalized.push_back(n->get_id());
        m_jointly_internalized.push_back(xor3_node->get_id());

        ctx.internalize(xor3_node);

        literal r = expr2literal(n);
        literal s = expr2literal(xor3_node);
        literal l1 = expr2literal(l1_expr);
        literal l2 = expr2literal(l2_expr);
        literal l3 = expr2literal(l3_expr);

        add_clause(l1, l2, l3, ~s);
        add_clause(~l1, ~l2, ~l3, s);
        add_clause(~l3, s, r);
        add_clause(l1, l2, ~r);
        add_clause(l1, ~l2, l3, s);
    ...
    void solver::internalize_xor3(app* n) {
        ...
        app* carry_node = m.mk_app(bv.get_fid(), OP_CARRY, l1_expr, l2_expr, l3_expr);

        ctx.push(restore_vector(m_jointly_internalized));
        m_jointly_internalized.push_back(n->get_id());
        m_jointly_internalized.push_back(carry_node->get_id());

        ctx.internalize(carry_node);

        literal s = expr2literal(n);
        literal r = expr2literal(carry_node);
        literal l1 = expr2literal(l1_expr);
        literal l2 = expr2literal(l2_expr);
        literal l3 = expr2literal(l3_expr);

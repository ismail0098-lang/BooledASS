...
#include "sat/booledass_sparse_tile.h"

/* Forward declarations for persistent GPU BCP API (booledass_kernels.cu) */
extern "C" GpuBcpPersistentState* gpu_bcp_init_persistent(
    uint32_t num_vars, uint32_t num_clauses,
    uint32_t M_pad, uint32_t K_pad, uint32_t num_active_tiles,
    const int8_t* active_tiles_data, const int8_t* active_tiles_abs_data,
    const uint16_t* h_lits, const sat::TileCoord* active_tiles_coords,
    const uint32_t* m_tile_start);
extern "C" int gpu_bcp_launch_async(GpuBcpPersistentState* state, const int8_t* h_assignment);
extern "C" bool gpu_bcp_poll_status(GpuBcpPersistentState* state);
extern "C" bool gpu_bcp_is_async_in_flight(GpuBcpPersistentState* state);
extern "C" int8_t* gpu_bcp_get_assignment_buffer(GpuBcpPersistentState* state);
extern "C" int gpu_bcp_retrieve_results(
    GpuBcpPersistentState* state,
    int32_t* out_unit_vars,
    int8_t*  out_unit_pols,
    uint32_t* out_count);
extern "C" void gpu_bcp_cleanup_persistent(GpuBcpPersistentState* state);
...
#define CONFLICT_CLEANUP() {                    \
    ...
    lbool solver::check(unsigned num_lits, literal const* lits) {
        ...
        try {
            ...
            // GPU BCP intercept: try GPU-accelerated BCP before CPU search
            lbool gpu_res = gpu_solve();
            if (gpu_res != l_undef) {
                return gpu_res;
            }
    ...
    lbool solver::basic_search() {
        ...
        while (is_sat == l_undef && !should_cancel()) {
            if (inconsistent()) is_sat = resolve_conflict_core();
                ...
                // 1. If GPU BCP is in flight, poll its status
                if (gpu_bcp_is_async_in_flight(m_gpu_state) && (++m_gpu_poll_tick & 2047) == 0) {
                    if (gpu_bcp_poll_status(m_gpu_state)) {
                        if (scope_lvl() < m_gpu_launch_level) {
                            // CPU state backtracked or restarted. Discard results.
                            IF_VERBOSE(2, verbose_stream() << "[GPU-CDCL] Discarded async results (stale due to CPU backtrack/restart)\n";);
                        } else {
                            // Retrieve results and inject
                            if (gpu_retrieve_and_inject()) {
                                continue; // loop back to CPU propagate/conflict handling
                            }
                        }
                    }
                }

                // 2. If GPU is NOT in flight, and CPU is stable (not inconsistent, no pending CPU propagate),
                //    and we met the throttle interval, launch a new async GPU BCP!
                if (!gpu_bcp_is_async_in_flight(m_gpu_state) && !inconsistent() && m_qhead == m_trail.size()
                    && m_gpu_decisions_since_call >= m_gpu_throttle_interval) {

                    m_gpu_decisions_since_call = 0;
                    m_gpu_launch_conflict_count = m_stats.m_conflict;
                    m_gpu_launch_level = scope_lvl();

                    gpu_launch_async();
                }

                // Fall through to other CPU search steps
                if (do_cleanup(false)) continue;
                ...
                else m_gpu_decisions_since_call++;
            }
            ...
            else m_gpu_decisions_since_call++;  /* decision was made */
        }
        return is_sat;
    }
...
//        return m_rephase.should_apply(m_conflicts_since_init);
    ...
    */
    ...
    extern "C" int gpu_bcp_solve_sparse(
        unsigned num_vars,
        unsigned num_clauses,
        unsigned M_pad,
        unsigned K_pad,
        unsigned num_active_tiles,
    ...
    lbool solver::gpu_solve() {
        svector<bin_clause> bins;
        collect_bin_clauses(bins, true, false);
        ...
        unsigned num_vars = this->num_vars();
        unsigned num_clauses = bins.size() + num_nary;

        if (num_vars == 0 || num_clauses == 0) {
            return l_undef;
        }

        // Bypass GPU for small instances (PCIe/setup overhead) where CPU is faster
        unsigned gpu_threshold = 200000;
        const char* env_threshold = getenv("Z3_GPU_THRESHOLD");
        if (env_threshold) {
            gpu_threshold = (unsigned)atoi(env_threshold);
        }
        if (num_clauses < gpu_threshold) {
            IF_VERBOSE(0, verbose_stream() << "[GPU] Bypassed. clauses " << num_clauses << " < " << gpu_threshold << "\n";);
            return l_undef;
        }

        IF_VERBOSE(0, verbose_stream() << "[GPU] Launching solver for " << num_clauses << " clauses, " << num_vars << " variables...\n";);

        // Build Compressed Blocked Tile layout directly (sparse)
        CompressedTileLayout ctl = build_compressed_tile_layout_direct(num_vars, bins, m_clauses, m_learned);

        int res = gpu_bcp_solve_sparse(
            num_vars,
            num_clauses,
            ctl.M_pad,
            ctl.K_pad,
            ctl.num_active_tiles,
            ctl.A,
            ctl.A_abs,
            ctl.num_lits,
            ctl.active_tiles,
        ...
        IF_VERBOSE(0, verbose_stream() << "[GPU] One-shot solver result: " << res << "\n";);

        // If one-shot didn't solve, initialize persistent state for CDCL-integrated BCP
        if (res == -1 && !m_gpu_state) {
            gpu_init_persistent_from_layout(ctl, num_vars, num_clauses);
        }

        free_compressed_tile_layout(ctl);

        IF_VERBOSE(0, verbose_stream() << "[GPU] Solver finished with result: " << res << "\n";);
    ...
    /* ═══════════════════════════════════════════════════════════════
     *  gpu_init_persistent_from_layout — sets up persistent GPU state
     *  from an already-built CompressedTileLayout.
     * ═══════════════════════════════════════════════════════════════ */
    void solver::gpu_init_persistent() {
        if (m_gpu_state || m_gpu_tried_init) return;
        m_gpu_tried_init = true;

        svector<std::pair<literal, literal>> bins;
        collect_bin_clauses(bins, true, false);

        unsigned nv = this->num_vars();
        unsigned num_nary = 0;
        for (clause* c : m_clauses) {
            if (!c->was_removed()) num_nary++;
        }
        for (clause* c : m_learned) {
            if (!c->was_removed()) num_nary++;
        }
        unsigned nc = bins.size() + num_nary;
        ...
        if (nc < gpu_threshold || nv == 0) return;

        CompressedTileLayout ctl = build_compressed_tile_layout_direct(nv, bins, m_clauses, m_learned);
        gpu_init_persistent_from_layout(ctl, nv, nc);
        free_compressed_tile_layout(ctl);
    }

    void solver::gpu_init_persistent_from_layout(CompressedTileLayout const& ctl,
                                                  unsigned nv, unsigned nc) {
        m_gpu_state = gpu_bcp_init_persistent(
            nv, nc,
            ctl.M_pad, ctl.K_pad,
            ctl.num_active_tiles,
            ctl.A, ctl.A_abs,
            ctl.num_lits, ctl.active_tiles, ctl.m_tile_start
        );

        if (m_gpu_state) {
            // Save variable permutation for assignment sync
            m_gpu_K_pad = ctl.K_pad;
            m_gpu_var_perm = (uint32_t*)malloc(nv * sizeof(uint32_t));
            m_gpu_var_inv  = (uint32_t*)malloc(nv * sizeof(uint32_t));
            if (ctl.var_perm && ctl.var_inv) {
                memcpy(m_gpu_var_perm, ctl.var_perm, nv * sizeof(uint32_t));
                memcpy(m_gpu_var_inv,  ctl.var_inv,  nv * sizeof(uint32_t));
            } else {
                // Identity permutation if none provided
                for (unsigned i = 0; i < nv; ++i) {
                    m_gpu_var_perm[i] = i;
                    m_gpu_var_inv[i]  = i;
                }
            }
            m_gpu_conflicts_at_init = m_stats.m_conflict;
            unsigned min_throttle = m_gpu_K_pad / 2048;
            if (min_throttle < 8) min_throttle = 8;
            m_gpu_throttle_interval = min_throttle * 2;
            m_gpu_tried_init = true;
            if (!m_gpu_unit_vars) m_gpu_unit_vars = (int32_t*)malloc(65536 * sizeof(int32_t));
            if (!m_gpu_unit_pols) m_gpu_unit_pols = (int8_t*)malloc(65536 * sizeof(int8_t));
            IF_VERBOSE(0, verbose_stream() << "[GPU-CDCL] Persistent state initialized. "
                       << nc << " clauses, " << nv << " vars, K_pad=" << m_gpu_K_pad << "\n";);
        }
    }

    /* ═══════════════════════════════════════════════════════════════
     *  gpu_propagate — called from basic_search() after CPU BCP
     *  exhausts its propagation queue.
     *
     *  1. Build assignment vector in GPU variable order
     *  2. Call Tensor Core GEMM-based BCP on GPU
     *  3. Inject discovered units into CPU trail
     *  4. Prefetch CPU cache-hot data to mitigate GPU cache pollution
     *
     *  Returns true if new units were injected (caller should re-run
     *  CPU propagation).
     * ═══════════════════════════════════════════════════════════════ */
    void solver::gpu_launch_async() {
        if (!m_gpu_state) return;

        unsigned nv = this->num_vars();
        int8_t* h_b = gpu_bcp_get_assignment_buffer(m_gpu_state);
        if (!h_b) return;

        // Zero out the assignment buffer using fast memset
        memset(h_b, 0, m_gpu_K_pad);

        for (unsigned v = 0; v < nv; ++v) {
            uint32_t pv = m_gpu_var_inv[v];  // permuted index for GPU
            if (pv >= m_gpu_K_pad) continue;

            lbool val = value(literal(v, false));
            if (val == l_true) {
                h_b[pv] = 1;
            } else if (val == l_false) {
                h_b[pv] = -1;
        ...
        gpu_bcp_launch_async(m_gpu_state, nullptr);
    }

    bool solver::gpu_retrieve_and_inject() {
        if (!m_gpu_state || !m_gpu_unit_vars || !m_gpu_unit_pols) return false;

        unsigned nv = this->num_vars();
        uint32_t unit_count = 0;

        int conflict = gpu_bcp_retrieve_results(
            m_gpu_state, m_gpu_unit_vars, m_gpu_unit_pols, &unit_count
        );

        // Prefetch CPU cache-hot data after GPU DMA to mitigate cache pollution
#if defined(_MSC_VER) && !defined(_M_ARM) && !defined(_M_ARM64)
        if (m_trail.size() > 0) {
            _mm_prefetch((const char*)m_assignment.data(), _MM_HINT_T0);
            _mm_prefetch((const char*)m_justification.data(), _MM_HINT_T0);
            _mm_prefetch((const char*)m_trail.data() + m_trail.size() * sizeof(literal) - 64, _MM_HINT_T0);
        }
#endif

        if (conflict) {
            IF_VERBOSE(2, verbose_stream() << "[GPU-CDCL] Conflict detected by GPU\n";);
        }

        // Move 2: Use the GPU as a "Conflict Generator"
        // Scan for conflicts first (GPU says lit must be true, but CPU trail has it false)
        bool conflict_found = false;
        for (uint32_t i = 0; i < unit_count; ++i) {
            int32_t perm_var = m_gpu_unit_vars[i];
            int8_t  pol      = m_gpu_unit_pols[i];

            if (perm_var < 0 || (uint32_t)perm_var >= m_gpu_K_pad) continue;

            uint32_t orig_var = m_gpu_var_perm[perm_var];
            if (orig_var >= nv) continue;

            literal lit = literal(orig_var, pol < 0);

            if (value(lit) == l_false) {
                // GPU says lit must be true, but CPU trail says it is false.
                // Trigger a binary conflict clause (lit | ~d) where d is the decision literal at lit's level.
                unsigned L = lvl(lit.var());
                if (L == 0) {
                    set_conflict(justification(0), lit);
                } else {
                    literal d = scope_literal(L - 1);
                    set_conflict(justification(L, ~d), lit);
                }
                conflict_found = true;
                break;
            }
        ...
        if (conflict_found) {
            m_gpu_dry_streak = 0;
            // A conflict is extremely productive; reduce throttle interval to call GPU more frequently
            unsigned min_throttle = m_gpu_K_pad / 2048;
            if (min_throttle < 8) min_throttle = 8;
            if (m_gpu_throttle_interval > min_throttle) {
                m_gpu_throttle_interval = m_gpu_throttle_interval / 2;
                if (m_gpu_throttle_interval < min_throttle) m_gpu_throttle_interval = min_throttle;
            }
            IF_VERBOSE(2, verbose_stream() << "[GPU-CDCL] Conflict generated by GPU, throttle: " 
                       << m_gpu_throttle_interval << "\n";);
            return true; // Return true to trigger CPU conflict resolution in basic_search()
        }

        // If no conflict and no units, we are done! Return false immediately.
        if (unit_count == 0) {
            // Adaptive throttle: GPU found nothing, back off
            m_gpu_dry_streak++;
            if (m_gpu_dry_streak >= 3 && m_gpu_throttle_interval < 65536) {
                m_gpu_throttle_interval = std::min(m_gpu_throttle_interval * 2, 65536u);
            }
            return false;
        }

        // Move 1: Filter Injections by VSIDS Activity
        // Compute maximum activity in the current solver state to establish threshold
        unsigned max_act = 0;
        for (unsigned v = 0; v < nv; ++v) {
            if (m_activity[v] > max_act) {
                max_act = m_activity[v];
            }
        }
        // Adaptive threshold divisor: starts at 16, increases as dry streak grows
        unsigned divisor = 16 << (m_gpu_dry_streak / 2);
        if (divisor > 4096) divisor = 4096;
        unsigned threshold = max_act / divisor;
        if (threshold > 500) threshold = 500;

        unsigned total_unassigned = 0;
        unsigned injected = 0;
        for (uint32_t i = 0; i < unit_count; ++i) {
            int32_t perm_var = m_gpu_unit_vars[i];
            int8_t  pol      = m_gpu_unit_pols[i];
            ...
            if (value(lit) == l_undef) {
                total_unassigned++;
                if (m_activity[orig_var] >= threshold) {
                    assign_core(lit, justification(scope_lvl()));
                    injected++;
                }
            }
        }
        ...
        if (injected > 0) {
            m_gpu_units_injected += injected;
            m_gpu_dry_streak = 0;
            unsigned min_throttle = m_gpu_K_pad / 2048;
            if (min_throttle < 8) min_throttle = 8;
            if (injected > 50 && m_gpu_throttle_interval > min_throttle) {
                m_gpu_throttle_interval = m_gpu_throttle_interval / 2;
                if (m_gpu_throttle_interval < min_throttle) m_gpu_throttle_interval = min_throttle;
            }
            IF_VERBOSE(2, verbose_stream() << "[GPU-CDCL] Injected " << injected << "/" << total_unassigned 
                       << " active units (max_act: " << max_act << ", threshold: " << threshold 
                       << ", calls: " << m_gpu_propagation_count << ", throttle: " << m_gpu_throttle_interval << ")\n";);
            return true;  // caller should re-run CPU propagate
        }
        ...
        IF_VERBOSE(2, verbose_stream() << "[GPU-CDCL] Dry call: " << total_unassigned << " units filtered out "
                   << "(max_act: " << max_act << ", threshold: " << threshold 
                   << ", dry streak: " << m_gpu_dry_streak << ", throttle: " << m_gpu_throttle_interval << ")\n";);

        return false;
    }

    /* ═══════════════════════════════════════════════════════════════
     *  gpu_cleanup — free persistent GPU state
     * ═══════════════════════════════════════════════════════════════ */
    void solver::gpu_cleanup() {
        if (m_gpu_state) {
            IF_VERBOSE(1, verbose_stream() << "[GPU-CDCL] Cleanup: "
                       << m_gpu_propagation_count << " GPU calls, "
                       << m_gpu_units_injected << " units injected\n";);
            gpu_bcp_cleanup_persistent(m_gpu_state);
            m_gpu_state = nullptr;
        }
        if (m_gpu_var_perm) { free(m_gpu_var_perm); m_gpu_var_perm = nullptr; }
        if (m_gpu_var_inv)  { free(m_gpu_var_inv);  m_gpu_var_inv  = nullptr; }
        if (m_gpu_unit_vars) { free(m_gpu_unit_vars); m_gpu_unit_vars = nullptr; }
        if (m_gpu_unit_pols) { free(m_gpu_unit_pols); m_gpu_unit_pols = nullptr; }
    }

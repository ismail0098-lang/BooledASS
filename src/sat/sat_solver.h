...
/* Forward declaration for GPU persistent BCP state (defined in booledass_kernels.cu) */
...
namespace sat {
    struct CompressedTileLayout;  /* forward decl for GPU persistent BCP */
    ...
    */
    ...
    public:
        ...
        lbool gpu_solve();
        void  gpu_init_persistent();
        void  gpu_launch_async();
        bool  gpu_retrieve_and_inject();
        void  gpu_cleanup();
    private:
        void  gpu_init_persistent_from_layout(CompressedTileLayout const& ctl, unsigned nv, unsigned nc);
    public:
    ...
    protected:
        ...
        /* GPU persistent BCP state */
        GpuBcpPersistentState* m_gpu_state = nullptr;
        uint32_t*  m_gpu_var_perm = nullptr;   /* permuted_var → original_var */
        uint32_t*  m_gpu_var_inv  = nullptr;   /* original_var → permuted_var */
        uint32_t   m_gpu_K_pad = 0;
        unsigned   m_gpu_propagation_count = 0;
        unsigned   m_gpu_units_injected = 0;
        unsigned   m_gpu_conflicts_at_init = 0;
        bool       m_gpu_tried_init = false;
        /* Adaptive GPU throttle: call GPU every m_gpu_throttle_interval decisions */
        unsigned   m_gpu_throttle_interval = 8;   /* decisions between GPU calls */
        unsigned   m_gpu_decisions_since_call = 0; /* decisions since last GPU call */
        unsigned   m_gpu_dry_streak = 0;           /* consecutive calls with 0 units */
        unsigned   m_gpu_launch_conflict_count = 0;
        unsigned   m_gpu_launch_level = 0;
        unsigned   m_gpu_poll_tick = 0;
        int32_t*   m_gpu_unit_vars = nullptr;
        int8_t*    m_gpu_unit_pols = nullptr;
